...
void CGameContext::SendChat(int ChatterClientID, int Mode, int To, const char *pText)
{
	...
	if(Mode == CHAT_ALL)
	{
		...
		int Team = m_apPlayers[ChatterClientID]->GetTeam();
		if(!m_pController->IsTeamplay() && Team == TEAM_RED)
			Team = NUM_TEAMS;
		/* # Discord # */
		Discord()->LogChat(Team, Server()->ClientName(ChatterClientID), pText);
		/* # Discord # */
	}
	else if(Mode == CHAT_TEAM)
	{
...
/* # Discord # */
void CGameContext::SendChatFromDiscord(const char *Nickname, const char *pText)
{
	int FakeID = MAX_PLAYERS + 1;

	CNetMsg_Sv_ClientInfo NewClientInfoMsg;
	NewClientInfoMsg.m_ClientID = FakeID;
	NewClientInfoMsg.m_Local = 0;
	NewClientInfoMsg.m_Team = TEAM_SPECTATORS;
	NewClientInfoMsg.m_pName = Nickname;
	NewClientInfoMsg.m_pClan = "Discord";
	NewClientInfoMsg.m_Country = 0;
	NewClientInfoMsg.m_Silent = true;

	for(int i = 0; i < NUM_SKINPARTS; i++)
	{
		NewClientInfoMsg.m_apSkinPartNames[i] = "default";
		NewClientInfoMsg.m_aUseCustomColors[i] = 0;
	}

	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		if(i == FakeID || !m_apPlayers[i] || (!Server()->ClientIngame(i) && !m_apPlayers[i]->IsDummy()))
			continue;
	...
	CNetMsg_Sv_Chat Msg;
	Msg.m_Mode = CHAT_ALL;
	Msg.m_ClientID = FakeID;
	Msg.m_pMessage = pText;
	Msg.m_TargetID = -1;

	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), "%d:%d:%s: %s", CHAT_ALL, TEAM_SPECTATORS, Nickname, pText);
	Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "chat|Discord", aBuf);

	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, -1);


	CNetMsg_Sv_ClientDrop Drop;
	Drop.m_ClientID = FakeID;
	Drop.m_pReason = "";
	Drop.m_Silent = true;
	Server()->SendPackMsg(&Drop, MSGFLAG_VITAL|MSGFLAG_NORECORD, -1);
}
/* # Discord # */
...
{
	/* # Discord # */
	Discord()->LogEnter(Server()->ClientName(ClientID));
	/* # Discord # */
...
{
	/* # Discord # */
	Discord()->LogExit(Server()->ClientName(ClientID));
	/* # Discord # */
